<?php
	...
	if(ISSET($_POST['save_d'])){
		...
		$conn = new mysqli("localhost", "cdpqjbjgnr", "jtRvWu7a7A", "cdpqjbjgnr") or die(mysql_error());
		$conn->query("INSERT INTO `dental` (date, dentist, tooth, itr_no, user_id, month, year) VALUES('$date', '$dentist', '$tooth', '$itr_no', '$user_id', '$month', '$year')") or die(mysqli_error());
	...
	if(ISSET($_POST['save_f'])){
		...
		$conn->query("INSERT INTO `fecalisys` (date_of_request, requested_by, color, consistency, pus_cells, rbc, fat_globules, occult_blood, others_chem, ova, larva, adult_forms, cyst, trophozoites, others_pro, remarks, date_reported, pathologist, medical_technologist, itr_no, user_id, month, year) VALUES('$date_of_request', '$requested_by', '$color', '$consistency', '$pus_cells', '$rbc', '$fat_globules', '$occult_blood', '$others_chem', '$ova', '$larva', '$adult_forms', '$cyst', '$trophozoites', '$others_pro', '$remarks', '$date_reported', '$pathologist', '$medical_technologist', '$itr_no', '$user_id', '$month', '$year')") or die(mysqli_error());
	...
	if(ISSET($_POST['save_u'])){
		...
		$year = date("Y", strtotime("+8 HOURS"));
		$conn = new mysqli("localhost", 'cdpqjbjgnr', 'jtRvWu7a7A', 'cdpqjbjgnr') or die(mysqli_error());
		$conn->query("INSERT INTO `urinalysis` (date_of_request, color, transparency, specific_gravity, ph, sugar, protein, pregnancy_test, pus_cells, rbc, cast, urates, uric_acid, cal_ox, epith_cells, mucus_threads, others, pathologist, medical_technologist, itr_no, user_id, month, year) VALUES('$date_of_request', '$color', '$transparency', '$specific_gravity', '$ph', '$sugar', '$protein', '$pregnacy_test', '$pus_cells', '$rbc', '$cast', '$urates', '$uric_acid', '$cal_ox', '$epith_cells', '$mucus_threads', '$others', '$pathologist', '$medical_technologist', '$itr_no', '$user_id', '$month', '$year')") or die(mysqli_error());
	...
	if(ISSET($_POST['save_r'])){
		...
		$year = date("Y", strtotime("+8 HOURS"));
		$conn = new mysqli("localhost", 'cdpqjbjgnr', 'jtRvWu7a7A', 'cdpqjbjgnr') or die(mysqli_error());
		$conn->query("INSERT INTO `rehabilitation` (diagnosis, type_of_disability, subjective, objective, assessment, plan, date, itr_no, user_id, month, year) VALUES('$diagnosis', '$type_of_disability', '$subjective', '$objective', '$assessment', '$plan', '$date', '$itr_no', '$user_id', '$month', '$year')") or die(mysqli_error());
	...
	if(ISSET($_POST['save_x'])){
		...
		$year = date("Y", strtotime("+8 HOURS"));
		$conn = new mysqli("localhost", 'cdpqjbjgnr', 'jtRvWu7a7A', 'cdpqjbjgnr') or die(mysqli_error());
		$conn->query("INSERT INTO `radiological` (case_no, referred_by, clinical_impression, room_bed_no, type_of_examination, date_taken, radiologist, itr_no, user_id, month, year) VALUES('$case_no', '$referred_by', '$clinical_impression', '$room_bed_no', '$type_of_examination', '$date_taken', '$radiologist', '$itr_no', '$user_id', '$month', '$year')") or die(mysqli_error());
	...
	if(ISSET($_POST['save_birth'])){
		...
		$year = date("Y", strtotime("+8 HOURS"));
		$conn = new mysqli("localhost", "cdpqjbjgnr", "jtRvWu7a7A", "cdpqjbjgnr") or die(mysqli_error());
		$conn->query("INSERT INTO `birthing` (date, time, chief_complaint, history, lmp, edc, aog, g, p, b1, b2, b3, b4, bp1, bp2, pr, rr, t, head_neck, chest, heart, abdomen, fhb, loc, extremities, vulva, vagina, cervix, uterus, bow, presentation, vaginal_discharge, staff, itr_no, user_id, month, year) VALUES('$date', '$time', '$chief_complaint', '$history', '$lmp', '$edc', '$aog', '$g', '$p', '$b1', '$b2', '$b3', '$b4', '$bp1', '$bp2', '$pr', '$rr', '$t', '$head_neck', '$chest', '$heart', '$abdomen', '$fhb', '$loc', '$extremities', '$vulva', '$vagina', '$cervix', '$uterus', '$bow', '$presentation', '$vaginal_discharge', '$staff', '$itr_no', '$user_id', '$month', '$year')") or die(mysqli_error());
