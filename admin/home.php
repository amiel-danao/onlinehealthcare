...
<?php
	...
	$qfecalysis = $conn->query("SELECT COUNT(*) as total FROM `fecalisys` WHERE `year` = '$date'") or die(mysqli_error());

	$ffecalysis = $qfecalysis->fetch_array();
	$qmaternity = $conn->query("SELECT COUNT(*) as total FROM `birthing` `prenatal` WHERE `year` = '$date'") or die(mysqli_error());

	$fmaternity = $qmaternity->fetch_array();
	$qhematology = $conn->query("SELECT COUNT(*) as total FROM `hematology` WHERE `year` = '$date'") or die(mysqli_error());

	$fhematology = $qhematology->fetch_array();
	$qdental = $conn->query("SELECT COUNT(*) as total FROM `dental` WHERE `year` = '$date'") or die(mysqli_error());

	$fdental = $qdental->fetch_array();
	$qxray = $conn->query("SELECT COUNT(*) as total FROM `radiological` WHERE `year` = '$date'") or die(mysqli_error());

	$fxray = $qxray->fetch_array();
	$qrehab = $conn->query("SELECT COUNT(*) as total FROM `rehabilitation` WHERE `year` = '$date'") or die(mysqli_error());

	$frehab = $qrehab->fetch_array();
	$qsputum = $conn->query("SELECT COUNT(*) as total FROM `sputum` WHERE `year` = '$date'") or die(mysqli_error());

	$fsputum = $qsputum->fetch_array();
	$qurinalysis = $conn->query("SELECT COUNT(*) as total FROM `urinalysis` WHERE `year` = '$date'") or die(mysqli_error());
...
<body>
	<div class = "navbar navbar-default navbar-fixed-top">
		<label class = "navbar-brand" color="red"> Patient Record System In Fabella </label>
		<?php 
	...
	<div id = "sidebar">
			<ul id = "menu" class = "nav menu">
				...
				<li><a href = ""><i class = "glyphicon glyphicon-cog"></i>All Accounts</a>
					<ul>
						<li><a href = "admin.php"><i class = "glyphicon glyphicon-cog"></i> Admins & Users</a></li>
						<li><a href = "user.php"><i class = "glyphicon glyphicon-cog"></i> User</a></li>
					</ul>
				</li>
				<li><li><a href = "patient.php"><i class = "glyphicon glyphicon-user"></i> Patient List</a></li>
				<li></li></li>
				<li><a href = ""><i class = "glyphicon glyphicon-folder-close"></i>&nbsp;Laboratory Sections</a>
